...
export async function getUser({email, password}: Credentials) : Promise<UserDto>{
  const user = (await db.select().from(userSchema)).find(user => user.email == email)
  
  if(user == undefined){
    throw new Error("The user with this username does not exist");
  }
